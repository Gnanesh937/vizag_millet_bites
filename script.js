...
function renderProductsByCategory(category) {
  ...
  filtered.forEach(product => {
    ...
    div.querySelector('.card-add').addEventListener('click', ()=>{ addToCartWithQty(product.name, parseInt(selectEl.value,10)); updateStatus(product.name); });
    div.querySelector('.card-remove').addEventListener('click', ()=>{ removeFromCartWithQty(product.name, parseInt(selectEl.value,10)); updateStatus(product.name); });
...
function addToCartWithQty(productName, qty) {
  ...
  if(!product) return;
  if(!cart[productName]) cart[productName] = {product, quantity:0};
...
function updateStatus(productName) {
  ...
  if(cart[productName]) {
    ...
    if(item.product.type==='combo') s.textContent = `In cart: ${item.quantity} Pack${item.quantity>1?'s':''}`;
    else s.textContent = `In cart: ${item.quantity>=1000?(item.quantity/1000).toFixed(2)+' kg':item.quantity+' g'}`;
  } else s.textContent="";
}
...
function updateCartDisplay() {
  ...
  for(const name in cart){
    ...
    if(item.product.type==='combo'){
      ...
      qtyControls=`<button onclick="adjustCartItem('${name}', -1)">-</button>${item.quantity} Pack${item.quantity>1?'s':''}<button onclick="adjustCartItem('${name}',1)">+</button>`;
    } else {
      const unit = item.product.pricePer===250?250:100;
      itemTotal=(item.quantity/unit)*item.product.price;
      qtyControls=`<button onclick="adjustCartItem('${name}', -${item.product.minQty===250?250:100})">-</button>${item.quantity>=1000?(item.quantity/1000).toFixed(2)+' kg':item.quantity+' g'}<button onclick="adjustCartItem('${name}', ${item.product.minQty===250?250:100})">+</button>`;
    }
    total+=itemTotal;
    html+=`<div class="cart-item"><div class="cart-item-name">${name}</div><div class="cart-item-qty">${qtyControls}</div><div class="cart-item-price">₹${itemTotal.toFixed(2)}</div></div>`;
  }
...
function sendOrder() {
  ...
  let totalWeight=0, hasWeight=false, hasCombo=false, total=0;
  for(const productName in cart){
    const item = cart[productName];
    if(item.product.type==='combo'){ hasCombo=true; total+=item.quantity*item.product.price; }
    else{ hasWeight=true; totalWeight+=item.quantity; const unit=item.product.pricePer===250?250:100; total+=(item.quantity/unit)*item.product.price; }
  }
  if(hasWeight && !hasCombo && totalWeight<500){ alert("Minimum order for weight-based products is 500g!"); return; }
...
/* ---------- Payment Success / Screenshot Modal ---------- */
window.addEventListener("load",function(){
  ...
  if(successData){
    ...
    if(!modal){
      ...
      modal.innerHTML=`
        <div style="background:#fff; padding:20px; border-radius:12px; width:90%; max-width:450px; text-align:center; position:relative;">
          <button id="screenshotBtn" style="position:absolute;top:15px;right:15px;padding:10px 15px;border:none;border-radius:6px;cursor:pointer;background:#ff7043;color:#fff;">📸 Screenshot</button>
          <h2>Payment Successful!</h2>
          <div id="orderSummaryCapture" style="margin-top:20px; text-align:left;">
            <p>Thank you! Your order has been placed successfully.</p>
            <div id="orderSummary"></div>
          </div>
          <button id="okBtn" style="margin-top:20px; padding:10px 20px; border:none; border-radius:6px; cursor:pointer; background:#3399cc; color:#fff;">OK</button>
        </div>
      `;
    ...
    for(const productName in successData.cart){
      ...
      let qtyText=item.product.type==='combo'?`${item.quantity} Pack${item.quantity>1?'s':''}`:item.quantity>=1000?(item.quantity/1000).toFixed(2)+' kg':item.quantity+' g';
      summaryHTML+=`<li>${productName}: ${qtyText}</li>`;
    }
    summaryHTML+="</ul>";
    summaryHTML+=`<p><strong>💰 Total Paid:</strong> ₹${successData.total.toFixed(2)}</p>`;
    summaryHTML+=`<p><strong>🆔 Payment ID:</strong> ${successData.paymentId}</p>`;
    summaryHTML+=`<h3>📍 Delivery Details:</h3>`;
    summaryHTML+=`<p>${successData.customer.name}, ${successData.customer.phone}</p>`;
    summaryHTML+=`<p>${successData.customer.door}, ${successData.customer.street}, ${successData.customer.area}</p>`;
    summaryHTML+=`<p>${successData.customer.city}, ${successData.customer.state} - ${successData.customer.pincode}</p>`;
    summaryEl.innerHTML=summaryHTML;

    modal.style.display="flex";
    document.getElementById("okBtn").onclick=()=>{ modal.style.display="none"; window.location.href="index.html#home"; };
    document.getElementById("screenshotBtn").onclick=()=>{ html2canvas(document.getElementById("orderSummaryCapture")).then(canvas=>{ const link=document.createElement("a"); link.download="order_summary.png"; link.href=canvas.toDataURL(); link.click(); }); };
...
function openProductModal(product){
  ...
  let defaultQty;
  if(product.type==='combo'){ const quantities=[1,2,3,5]; defaultQty=quantities[0]; quantities.forEach(q=>{ const opt=document.createElement("option"); opt.value=q; opt.textContent=`${q} Pack${q>1?'s':''}`; if(q===defaultQty) opt.selected=true; select.appendChild(opt); }); }
  else{ const opts=product.minQty===250?[250,500,1000]:[100,250,500,1000]; defaultQty=opts[0]; opts.forEach(o=>{ const opt=document.createElement("option"); opt.value=o; opt.textContent=o===1000?'1KG':o+'g'; if(o===defaultQty) opt.selected=true; select.appendChild(opt); }); }

  document.getElementById("modalAddBtn").onclick=()=>{ addToCartWithQty(product.name,parseInt(select.value,10)); updateStatus(product.name); updateModalStatus(product.name); };
  document.getElementById("modalRemoveBtn").onclick=()=>{ removeFromCartWithQty(product.name,parseInt(select.value,10)); updateStatus(product.name); updateModalStatus(product.name); };
...
function updateModalStatus(productName){
  ...
  if(cart[productName]){
    const item=cart[productName];
    if(item.product.type==='combo') statusEl.textContent=`In cart: ${item.quantity} Pack${item.quantity>1?'s':''}`;
    else statusEl.textContent=`In cart: ${item.quantity>=1000?(item.quantity/1000).toFixed(2)+' kg':item.quantity+' g'}`;
  } else statusEl.textContent="Not in cart";
}

document.getElementById("closeProductModal").addEventListener("click",()=>{ document.getElementById("productModal").style.display="none"; });
document.getElementById("productModal").addEventListener("click",function(e){ if(e.target && e.target.id==='productModal'){ document.getElementById("productModal").style.display="none"; } });
